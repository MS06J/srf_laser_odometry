 ...
 * SRF is a fast and precise method to estimate the planar motion of a lidar from consecutive range
 *scans. SRF presents a dense method for estimating planar motion with a laser scanner. Starting
 *from a symmetric representation of geometric consistency between scans, we derive a precise range
 *flow constraint and express the motion of the scan observations as a function of the rigid motion
 *of the scanner. In contrast to existing techniques, which align the incoming scan with either the
 *previous one or the last selected keyscan, we propose a combined and efficient formulation to
 *jointly align all these three scans at every iteration. This new formulation preserves the
 *advantages of keyscan-based strategies but is more robust against suboptimal selection of keyscans
 *and the presence of moving objects.
 *
...
inline Scalar getYaw(const Eigen::Transform<Scalar, 2, 1>& r) {
...
void SRF_RefS::initialize(unsigned int size, float FOV_rad, unsigned int odo_method) {
  ...
  width = size;
  fovh = FOV_rad * size /
         (size - 1);  // Exact for simulation, but I don't know how the datasets are given...
  ...
  const unsigned int pyr_levels =
      std::round(std::log2(std::round(float(width) / float(cols)))) + ctf_levels;
  range_1.resize(pyr_levels);
  range_2.resize(pyr_levels);
  range_3.resize(pyr_levels);
  range_12.resize(pyr_levels);
  range_13.resize(pyr_levels);
  xx_1.resize(pyr_levels);
  xx_2.resize(pyr_levels);
  xx_3.resize(pyr_levels);
  xx_12.resize(pyr_levels);
  xx_13.resize(pyr_levels);
  yy_1.resize(pyr_levels);
  yy_2.resize(pyr_levels);
  yy_3.resize(pyr_levels);
  yy_12.resize(pyr_levels);
  yy_13.resize(pyr_levels);
  range_warped.resize(pyr_levels);
  xx_warped.resize(pyr_levels);
  yy_warped.resize(pyr_levels);
  range_3_warpedTo2.resize(pyr_levels);
  xx_3_warpedTo2.resize(pyr_levels);
  yy_3_warpedTo2.resize(pyr_levels);

  for (unsigned int i = 0; i < pyr_levels; i++) {
    s = std::pow(2.f, int(i));
    cols_i = std::ceil(float(width) / float(s));

    range_1[i].resize(cols_i);
    range_2[i].resize(cols_i);
    range_3[i].resize(cols_i);
    range_12[i].resize(cols_i);
    range_13[i].resize(cols_i);
    range_1[i].fill(0.f);
    range_2[i].fill(0.f);
    range_3[i].fill(0.f);
    xx_1[i].resize(cols_i);
    xx_2[i].resize(cols_i);
    xx_3[i].resize(cols_i);
    xx_12[i].resize(cols_i);
    xx_13[i].resize(cols_i);
    xx_1[i].fill(0.f);
    xx_2[i].fill(0.f);
    xx_3[i].fill(0.f);
    yy_1[i].resize(cols_i);
    yy_2[i].resize(cols_i);
    yy_3[i].resize(cols_i);
    yy_12[i].resize(cols_i);
    yy_13[i].resize(cols_i);
    yy_1[i].fill(0.f);
    yy_2[i].fill(0.f);
    yy_3[i].fill(0.f);
    ...
    if (cols_i <= cols) {
      ...
      range_3_warpedTo2[i].resize(cols_i);
      xx_3_warpedTo2[i].resize(cols_i);
      yy_3_warpedTo2[i].resize(cols_i);
    }
  }
  ...
  dt_12.resize(cols);
  dt_13.resize(cols);
  dtita_12.resize(cols);
  dtita_13.resize(cols);
  weights_12.resize(cols);
  weights_13.resize(cols);
  null_12.resize(cols);
  null_13.resize(cols);
  null_12.fill(false);
  null_13.fill(false);
  cov_odo.setConstant(0.f);
  ...
  for (unsigned int i = 0; i < pyr_levels; i++) {
        ...
        for (unsigned int u = 0; u < cols_i; u++) {
          ...
          if ((u > 0) && (u < cols_i - 1)) {
            if (std::isfinite(dcenter) && dcenter > 0.f) {
              ...
              for (int l = -2; l < 3; l++) {
                const float abs_dif = std::abs(range_1[i_1](u2 + l) - dcenter);
                if (abs_dif < max_range_dif) {
                  const float aux_w = g_mask[2 + l] * (max_range_dif - abs_dif);
                  weight += aux_w;
    ...
    for (unsigned int u = 0; u < cols_i; u++) {
      if (range_1[i](u) > 0.f) {
        ...
        xx_1[i](u) = range_1[i](u) * std::cos(tita);
        yy_1[i](u) = range_1[i](u) * std::sin(tita);
      } else {
        xx_1[i](u) = 0.f;
        yy_1[i](u) = 0.f;
      }
...
void SRF_RefS::calculateCoord() {
  ...
  for (unsigned int u = 0; u < cols_i; u++) {
    ...
    if ((range_2[image_level](u) == 0.f) || (range_warped[image_level](u) == 0.f)) {
      ...
      if ((u > 0) && (u < cols_i - 1)) num_valid_range++;
    }
    ...
    if ((range_3_warpedTo2[image_level](u) == 0.f) || (range_warped[image_level](u) == 0.f)) {
      ...
      range_13[image_level](u) =
          0.5f * (range_3_warpedTo2[image_level](u) + range_warped[image_level](u));
...
void SRF_RefS::calculateRangeDerivatives() {
  ...
  for (unsigned int u = 0; u < cols_i - 1; u++) {
    ...
    if (dist_12 > 0.f) rtita_12(u) = sqrtf(dist_12);

    if (dist_13 > 0.f) rtita_13(u) = sqrtf(dist_13);
  }

  // Spatial derivatives
  for (unsigned int u = 1; u < cols_i - 1; u++) {
    dtita_12(u) = (rtita_12(u - 1) * (range_12[image_level](u + 1) - range_12[image_level](u)) +
                      rtita_12(u) * (range_12[image_level](u) - range_12[image_level](u - 1))) /
                  (rtita_12(u) + rtita_12(u - 1));
    dtita_13(u) = (rtita_13(u - 1) * (range_13[image_level](u + 1) - range_13[image_level](u)) +
                      rtita_13(u) * (range_13[image_level](u) - range_13[image_level](u - 1))) /
                  (rtita_13(u) + rtita_13(u - 1));
  }
...
void SRF_RefS::computeWeights() {
  ...
  //    const float kd = 1.f;
  //    const float k2d = 0.02f; //0.5 no, 0.3 no, 0.1 no, 0.05 no, 0.02 no (better between 0.2 and
  //    0.05) const float sensor_sigma = 100.f*4e-4f; //100.f*4e-4f, 200 is too much
  ...
  for (unsigned int u = 1; u < cols_i - 1; u++) {
    if (null_12(u) == false) {
      ...
      const float w_der =
          kd * (square(dt_12(u)) + square(dtita_12(u))) + k2d * square(dtita2) + sensor_sigma;
    ...
    if (null_13(u) == false) {
      //							Compute derivatives
      //-----------------------------------------------------------------------
      // const float ini_dtita = range_3_warpedTo2[image_level](u+1) -
      // range_3_warpedTo2[image_level](u-1); const float final_dtita =
      // range_warped[image_level](u+1) - range_warped[image_level](u-1); const float dtitat =
      // ini_dtita - final_dtita;
      const float dtita2 = dtita_13(u + 1) - dtita_13(u - 1);
      const float w_der =
          kd * (square(dt_13(u)) + square(dtita_13(u))) + k2d * square(dtita2) + sensor_sigma;

      weights_13(u) = sqrtf(1.f / w_der);
    }
  }

  const float max_w = std::max(weights_12.maxCoeff(), weights_13.maxCoeff());
...
void SRF_RefS::solveSystemQuadResiduals3Scans() {
  ...
  for (unsigned int u = 1; u < cols_i - 1; u++) {
    if (null_12(u) == false) {
      ...
      // Fill the matrix A
      A(cont, 0) =
          tw * (std::cos(tita) + dtita_12(u) * kdtita * std::sin(tita) / range_12[image_level](u));
      A(cont, 1) =
          tw * (std::sin(tita) - dtita_12(u) * kdtita * std::cos(tita) / range_12[image_level](u));
      A(cont, 2) = tw * (-yy_12[image_level](u) * std::cos(tita) +
                            xx_12[image_level](u) * std::sin(tita) - dtita_12(u) * kdtita);  //?????
      B(cont) = tw * (-dt_12(u));
    ...
    if (null_13(u) == false) {
      ...
      // Fill the matrix A
      A(cont, 0) =
          tw * (std::cos(tita) + dtita_13(u) * kdtita * std::sin(tita) / range_13[image_level](u));
      A(cont, 1) =
          tw * (std::sin(tita) - dtita_13(u) * kdtita * std::cos(tita) / range_13[image_level](u));
      A(cont, 2) = tw * (-yy_13[image_level](u) * std::cos(tita) +
                            xx_13[image_level](u) * std::sin(tita) - dtita_13(u) * kdtita);
      B(cont) = tw * (-dt_13(u));
  ...
  Eigen::MatrixXf AtA = A.adjoint() * A;
  Eigen::MatrixXf AtB = A.adjoint() * B;
  kai_loc_level = AtA.ldlt().solve(AtB);

  // Covariance matrix calculation
  Eigen::VectorXf res = A * kai_loc_level - B;
...
void SRF_RefS::solveSystemSmoothTruncQuad3Scans() {
  ...
  for (unsigned int u = 1; u < cols_i - 1; u++) {
    ...
    const float cos_tita = std::cos(tita);
    const float sin_tita = std::sin(tita);
    ...
    if (null_12(u) == false) {
      ...
      A(cont, 2) = tw * (-yy_12[image_level](u) * cos_tita + xx_12[image_level](u) * sin_tita -
                            dtita_12(u) * kdtita);
      B(cont) = tw * (-dt_12(u));
    ...
    if (null_13(u) == false) {
      ...
      A(cont, 2) = tw * (-yy_13[image_level](u) * cos_tita + xx_13[image_level](u) * sin_tita -
                            dtita_13(u) * kdtita);
      B(cont) = tw * (-dt_13(u));
  ...
  for (unsigned int k = 0; k < res.rows(); k++) aux_vector.push_back(res(k));
  std::sort(aux_vector.begin(), aux_vector.end());
  float res_median = aux_vector.at(res.rows() / 2);

  // Compute the median absolute deviation
  aux_vector.clear();
  for (unsigned int k = 0; k < res.rows(); k++) aux_vector.push_back(abs(res(k) - res_median));
  std::sort(aux_vector.begin(), aux_vector.end());
  ...
  for (unsigned int i = 0; i < res.rows(); i++) {
    if (std::abs(res(i)) < c)
      new_energy += 0.5f * square(res(i)) * (1.f - 0.5f * square(res(i) * inv_c));
    else
      new_energy += 0.25f * squared_c;
  }
  ...
  while ((new_energy < 0.995f * last_energy) && (iter < 10)) {
    ...
    for (unsigned int u = 1; u < cols_i - 1; u++) {
      if (null_12(u) == false) {
        float res_weight;
        if (std::abs(res(cont)) <= c)
          res_weight = 1.f - square(res(cont) * inv_c);
        else
          res_weight = 0.f;
...
void SRF_RefS::solveSystemSmoothTruncQuadOnly13() {
  ...
  for (unsigned int u = 1; u < cols_i - 1; u++) {
    if (null_13(u) == false) valid_here++;
  }

  A.resize(valid_here, 3);
  Aw.resize(valid_here, 3);
  B.resize(valid_here);
  Bw.resize(valid_here);
  unsigned int cont = 0;
  const float kdtita = float(cols_i) / fovh;
  const float inv_kdtita = 1.f / kdtita;
  ...
  // Compute the median absolute deviation
  aux_vector.clear();
  for (unsigned int k = 0; k < res.rows(); k++) aux_vector.push_back(std::abs(res(k) - res_median));
  std::sort(aux_vector.begin(), aux_vector.end());
...
void SRF_RefS::performWarping() {
  ...
  for (unsigned int i = 0; i <= level; i++) acu_trans = transformations[i] * acu_trans;

  Eigen::ArrayXf wacu(cols_i);
  wacu.fill(0.f);
  ...
  for (unsigned int j = 0; j < cols_i; j++) {
    if (range_1[image_level](j) > 0.f) {
      ...
      const float x_w = acu_trans(0, 0) * xx_1[image_level](j) +
                        acu_trans(0, 1) * yy_1[image_level](j) + acu_trans(0, 2);
      const float y_w = acu_trans(1, 0) * xx_1[image_level](j) +
                        acu_trans(1, 1) * yy_1[image_level](j) + acu_trans(1, 2);
...
void SRF_RefS::performBestWarping() {
  ...
  Eigen::ArrayXf x_trans(cols_i), y_trans(cols_i), u_trans(cols_i), range_trans(cols_i);
  x_trans.fill(0.f);
  y_trans.fill(0.f);
  range_trans.fill(0.f);
  range_warped[image_level].fill(0.f);
  ...
  for (unsigned int u = 0; u < cols_i; u++) {
    if (range_1[image_level](u) != 0.f) {
      // Transform point to the warped reference frame
      x_trans(u) = acu_trans(0, 0) * xx_1[image_level](u) + acu_trans(0, 1) * yy_1[image_level](u) +
                   acu_trans(0, 2);
      y_trans(u) = acu_trans(1, 0) * xx_1[image_level](u) + acu_trans(1, 1) * yy_1[image_level](u) +
                   acu_trans(1, 2);
      range_trans(u) = sqrtf(square(x_trans(u)) + square(y_trans(u)));
      const float tita_trans = std::atan2(y_trans(u), x_trans(u));
  ...
  for (unsigned int u = 0; u < cols_i - 1; u++) {
    if ((range_trans(u) == 0.f) || (range_trans(u + 1) == 0.f))
      ...
      const float range_l =
          (floorf(u_trans(u)) > floorf(u_trans(u + 1))) ? range_trans(u + 1) : range_trans(u);
      const float range_r =
          (floorf(u_trans(u)) > floorf(u_trans(u + 1))) ? range_trans(u) : range_trans(u + 1);
      const float u_trans_l =
          (floorf(u_trans(u)) > floorf(u_trans(u + 1))) ? u_trans(u + 1) : u_trans(u);
      const float u_trans_r =
          (floorf(u_trans(u)) > floorf(u_trans(u + 1))) ? u_trans(u) : u_trans(u + 1);
      const int u_l = std::min(floorf(u_trans(u)), floorf(u_trans(u + 1)));
      const int u_r = std::max(floorf(u_trans(u)), floorf(u_trans(u + 1)));

      for (unsigned int u_segment = u_l + 1;
           (u_segment <= u_r) && (u_segment < cols_i) && (u_segment >= 0); u_segment++) {
        const float range_interp =
            ((u_segment - u_trans_l) * range_r + (u_trans_r - u_segment) * range_l) /
            (u_trans_r - u_trans_l);
        if ((range_warped[image_level](u_segment) == 0.f) ||
            (range_interp < range_warped[image_level](u_segment)))
...
void SRF_RefS::warpScan3To2() {
  ...
  for (unsigned int i = 0; i < ctf_levels; i++) {
    ...
    image_level =
        ctf_levels - i + std::round(std::log2(std::round(float(width) / float(cols)))) - 1;
    ...
    for (unsigned int u = 0; u < cols_i; u++) {
      if (range_3[image_level](u) != 0.f) {
        // Transform point to the warped reference frame
        x_trans(u) = acu_trans_inv(0, 0) * xx_3[image_level](u) +
                     acu_trans_inv(0, 1) * yy_3[image_level](u) + acu_trans_inv(0, 2);
        y_trans(u) = acu_trans_inv(1, 0) * xx_3[image_level](u) +
                     acu_trans_inv(1, 1) * yy_3[image_level](u) + acu_trans_inv(1, 2);
        range_trans(u) = sqrtf(square(x_trans(u)) + square(y_trans(u)));
        const float tita_trans = std::atan2(y_trans(u), x_trans(u));
        u_trans(u) = kdtita * (tita_trans + 0.5f * fovh) - 0.5f;
      }
    ...
    for (unsigned int u = 0; u < cols_i - 1; u++) {
      if ((range_trans(u) == 0.f) || (range_trans(u + 1) == 0.f))
        ...
        for (unsigned int u_segment = u_l + 1;
          ...
          //                   if ((range_3_warpedTo2[image_level](u_segment) == 0.f)||(range_interp
          //                   < range_3_warpedTo2[image_level](u_segment)))
          //                       range_3_warpedTo2[image_level](u_segment) = range_interp;

          // Condition to obtain the structure of the environment (I retain the furthest point for
          // each pixel)
...
void SRF_RefS::odometryCalculation() {
  ...
  // Clear flag
  if (new_ref_scan == true) new_ref_scan = false;
...
void SRF_RefS::filterLevelSolution() {
  ...
  //-------------------------------------------------------------------------------------------------
  Eigen::Matrix3f Bii = eigensolver.eigenvectors();
  Eigen::Vector3f kai_b = Bii.colPivHouseholderQr().solve(kai_loc_level);

  // Second, we have to describe both the old linear and angular speeds in the "eigenvector" basis
  // too
  //-------------------------------------------------------------------------------------------------
  Eigen::Vector3f kai_loc_sub;
  ...
  for (unsigned int i = 0; i < 3; i++) {
    kai_b_fil(i, 0) =
        (kai_b(i, 0) + (cf * eigensolver.eigenvalues()(i, 0) + df) * kai_b_old(i, 0)) /
        (1.f + cf * eigensolver.eigenvalues()(i, 0) + df);
  ...
  Pose2d new_trans = Pose2d::Identity();
  new_trans.linear() = Eigen::Rotation2D<float>(rot).toRotationMatrix();

  Eigen::Matrix2f V = Eigen::Matrix2f::Identity();
  if (std::abs(rot) > 0.001f) {
    const float V1 = new_trans(1, 0) / rot;
    const float V2 = (1.f - new_trans(0, 0)) / rot;
    V << V1, -V2, V2, V1;
  }

  Eigen::Vector2f incr = V * Eigen::Vector2f(incrx, incry);

  new_trans.translation() << incr(0), incr(1);
...
void SRF_RefS::PoseUpdate() {
  ...
  //---------------------------------------------------
  Eigen::Matrix3f acu_trans = Eigen::Matrix3f::Identity();
  for (unsigned int i = 1; i <= ctf_levels; i++) acu_trans = transformations[i - 1] * acu_trans;
  ...
  if (acu_trans(0, 0) > 1.f)
    ...
    kai_loc(2) = std::acos(acu_trans(0, 0)) * sign(acu_trans(1, 0));

  float phi = getYaw(laser_pose);
  ...
  Pose2d pose_aux_2D = Pose2d::Identity();
  pose_aux_2D.linear() = Eigen::Rotation2D<float>(kai_loc(2)).toRotationMatrix();
...
void SRF_RefS::updateReferenceScan() {
  // Threshold small scanner (180, 360, 30 m): r = -14.92*t⁴ - 7.617*t³ + 2.307*t² - 0.3149*t +
  // 0.1054 Threshold big scanner (270, 541, 80 m): r = 3.072*t⁴ - 3.916*t³ + 0.1799*t² + 0.0883*t +
  // 0.2849 Threshold very big scanner (270, 1080, 30 m): r = -10.66*t⁴ + 11.81*t³ - 4.371*t² +
  // 0.5319*t + 0.2042 const float threshold = 0.5f;

  const float trans = sqrtf(square(overall_trans_prev(0, 2)) + square(overall_trans_prev(1, 2)));
  const float rot = std::abs(std::acos(overall_trans_prev(0, 0)));
  // printf("\n Trans = %f, rot = %f", trans, rot);

  float keyscan_out_region;
  const float trans_2 = square(trans);
  if (cols < 500)
    keyscan_out_region = -14.92 * square(trans_2) - 7.617 * trans_2 * trans + 2.307 * trans_2 -
                         0.3149 * trans + 0.1054 - rot;
  else
    keyscan_out_region = -10.66 * square(trans_2) + 11.81 * trans_2 * trans - 4.371 * trans_2 +
                         0.5319 * trans + 0.2042 - rot;
